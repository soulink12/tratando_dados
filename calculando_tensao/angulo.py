...
def calcular_derivada(vetor_inicial, x_increment):
    ...
    derivada = (-3 * vetor[i * x_increment] + 4 * vetor[(i + 1) * x_increment] - vetor[(i + 2) * x_increment]) / (x_increment * 2)
...
def criar_grafico_linhas_angulo():
    ...
    for i in range(int(n_pastas / 2), n_pastas, 1):
        ...
        angulo_max_min = 270 - (b_min - b_max) / (a_max - a_min)
    ...
    for i in range(int(n_pastas / 2), n_pastas, 1):
        ...
        globals()["ax3"][i].set_rlabel_position(-22.5)  # get radial labels away from plotted line
        globals()["ax3"][i].grid(False)
    plt.savefig(fr"D:\ultrassom\chapa g2-auto.png", dpi=300)
    plt.show()
...
def corrigir_angulos(angulo_max_minL11, angulo_max_minL12, angulo_max_minL2, angulo_max_minL31, angulo_max_minL32, intervalo_angulo):
    ...
    lista_de_listas_corrigido = [angulo_max_minL11_corrigido, angulo_max_minL12_corrigido, angulo_max_minL2_corrigido, angulo_max_minL31_corrigido, angulo_max_minL32_corrigido]
